...
const { default: SignIn } = await loadModule('SignIn.js')

test('SignIn renders streamlined auth layout', () => {
  const html = renderToString(
    React.createElement(MemoryRouter, null, React.createElement(SignIn))
  )

  assert.match(html, /auth-shell/)
  assert.match(html, /signin-card/)
